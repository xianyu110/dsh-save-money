...
DSH（DeepSeek Harness）**省钱插件** —— 自定义"暂停 / 继续"时间窗口，到点自动**暂停**（不是停止）正在执行的长任务，窗口结束自动恢复。应对大模型 API 的**峰谷分时计价**（如 DeepSeek 高峰 9:00–12:00、14:00–18:00 北京时间，空闲时段半价；**2026-08-23 起周末全天按低谷价**——插件的周窗口开关默认跳过周六日），也适用于分时电价、带宽错峰等任何"这个时间段不想让机器干活"的场景。
...
- **按模型档位决定是否省钱**：可以按模型档位选择哪些在窗口内暂停。设置面板用两行紧凑布局——**官方 API**（DeepSeek 官方）与**其他 API**（所有非官方 provider：opencode go/zen、中转、硅基流动等），每行 **flash / pro / vision** 三个开关（vision = `deepseek-v4-flash-vision-exp`），共 6 个开关。勾选 = 暂停（省钱）；不勾选 = **豁免**（窗口内也照常放行）。默认勾选官方三档、其他全豁免；你的改动持久化保存、绝不重置。任何模型名不含 flash/pro/vision 标记的模型（旧名 `chat`/`reasoner`、未知模型等）一律豁免——永远不会阻塞你的请求；
- **全局周窗口开关**：窗口列表上方一行——一 ☑️ 二 ☑️ 三 ☑️ 四 ☑️ 五 ☑️ 六 ⬜ 日 ⬜。只在勾选的星期执行省钱（默认**周一~周五**——按 2026-08-23 起的新峰谷规则，周末全天低谷价，周六日不暂停）。与每个窗口自带的星期过滤叠加：两者都满足窗口才生效；
...
ls                          # 查看插件打包文件名，比如 dsh-save-money-1.4.4.tgz
...
pnpm dsh plugin --profile web add ../dsh-save-money/plugin/dsh-save-money-1.4.4.tgz   # 文件名按上面 ls 的实际输出
...
cd plugin
npm pack                # 自动执行 prepare 构建；产出 dsh-save-money-1.4.4.tgz
```
...
```sh
scp dsh-save-money/plugin/dsh-save-money-1.4.4.tgz pi@<树莓派IP>:~/
```
