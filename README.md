...
ls                          # check the tarball name, e.g. dsh-save-money-1.4.2.tgz
...
pnpm dsh plugin --profile web add ../dsh-save-money/plugin/dsh-save-money-1.4.2.tgz   # use the name from `ls` above
...
cd plugin
npm pack                # runs the prepare build automatically; produces dsh-save-money-1.4.2.tgz
```
...
```sh
scp dsh-save-money/plugin/dsh-save-money-1.4.2.tgz pi@<pi-ip>:~/
```
...
pnpm dsh plugin --profile web add ~/dsh-save-money-1.4.2.tgz
# npx-launched (Step 0, option A) or globally installed dsh: works from any directory
npx @deepseek-ai/dsh plugin --profile web add ~/dsh-save-money-1.4.2.tgz
```
